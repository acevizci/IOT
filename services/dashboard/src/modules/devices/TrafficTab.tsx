...
import { useTopTalkers, useTrafficSummary, useProtocolBreakdown } from "../traffic/useTraffic";
import { formatBytes, protocolName } from "../traffic/format";
...
export function TrafficTab({ deviceId }: { deviceId: string }) {
  const [hours, setHours] = useState(1);
  const { data: summary } = useTrafficSummary(hours, deviceId);
  const { data: topTalkers, isLoading } = useTopTalkers(hours, deviceId);
  const { data: protocols } = useProtocolBreakdown(hours, deviceId);

  const hasNoData = summary && Number(summary.flow_count) === 0;

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-text-secondary">Bu cihazin (NetFlow exporter'i olarak) urettigi trafik.</p>
      ...
      {hasNoData ? (
        <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed border-border rounded-2xl">
          <Network size={28} className="text-text-muted mb-3" />
          <p className="text-sm font-medium mb-1">Bu cihaz icin trafik verisi yok</p>
          <p className="text-xs text-text-muted">Bu cihaz bir NetFlow exporter olarak yapilandirilmamis olabilir.</p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-[1.6fr_1fr] gap-4">
            <div className="bg-surface-2 border border-border rounded-xl">
              <p className="text-sm font-medium px-4 pt-3.5 pb-2">En cok trafik ureten IP ciftleri</p>
              {isLoading && <p className="text-sm text-text-secondary px-4 pb-4">Yukleniyor...</p>}
              ...
              {topTalkers?.length === 0 && <p className="text-sm text-text-muted px-4 py-6">Bu araliktA trafik verisi yok.</p>}
            </div>

            <div className="bg-surface-2 border border-border rounded-xl">
              <p className="text-sm font-medium px-4 pt-3.5 pb-2">Port / protokol dagilimi</p>
              {protocols?.map((p, i) => (
