...
// Zabbix'in "Geographical maps" özelliğine karşılık: koordinatı tanımlı cihazları
// gerçek bir dünya haritasında (OpenStreetMap karoları) gösterir. react-leaflet
// yerine saf leaflet kullanıldı -- React 19 ile peer-dependency riski almamak için.
// Pin'ler L.divIcon ile (SVG circleMarker değil) çiziliyor, çünkü CSS
// custom property'lerini (tema renkleri) doğrudan kullanabiliyoruz.
...
export function GeoMapPage() {
  ...
  useEffect(() => {
    ...
    const map = L.map(mapContainerRef.current).setView([41.0082, 28.9784], 5);
  ...
  return (
    <div>
      ...
      <div ref={mapContainerRef} className="w-full rounded-2xl border border-border" style={{ height: "70vh" }} />
    </div>
  );
}
