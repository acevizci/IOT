...
def main():
    ...
    existing_names = {t["name"] for t in existing} if status == 200 and existing else set()

    for t in templates:
        tname = t["name"]
        if tname in existing_names:
            print(f"[ATLA] '{tname}' zaten mevcut.")
            report["skipped_templates"].append({"name": tname, "reason": "zaten mevcut"})
            continue
        ...
        importable_items = [i for i in t.get("items", []) if i.get("type") in ("SNMP_AGENT", "SIMPLE", "HTTP_AGENT", "DEPENDENT")]
